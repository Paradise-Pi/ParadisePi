...
function lxPresetCard(presetArea, value, firstUniverse, lastUniverse, folderList) {
  const card = presetArea.addChild('<div class=\'card\' style=\'min-width: 350px;\'>');
  // Card Title
  card.addChild('<div class="card-header">' + (value.id == null ? '<strong>New Preset</strong>' : '<strong>Preset</strong> ' + value.id) + '</div>\n');
  // card Body + form
  const form = card.addChild('<div class=\'card-body\'>').addChild('<form class="preset-form" data-table="LXPreset">');
  // Name
  form.addChild('<div class="form-group"><label>Preset Name</label>\n<input class="form-control" name="name" type="text" value="' + value.name + '">\n</div>');
  // Folder
  const folderDiv = form.addChild('<div class="form-group">');
  folderDiv.addChild('<label>Preset Folder</label>');
  const folderSelect = folderDiv.addChild('<select class="form-control" name="folderId">');
...
function lxPresetFolderCard(presetArea, value, folderList) {
  const card = presetArea.addChild('<div class=\'card\'>');
  // Card Title
  card.addChild('<div class="card-header">' + (value.id == null ? '<strong>New Folder</strong>' : '<strong>Folder</strong> ' + value.id) + '</div>\n');
  // card Body + form
  const form = card.addChild('<div class=\'card-body\'>').addChild('<form class="preset-form" data-table="lxPresetFolders">');
  // Name
  form.addChild('<div class="form-group"><label>Folder Name</label>\n<input class="form-control" name="name" type="text" value="' + value.name + '">\n</div>');
  // Parent
  const folderDiv = form.addChild('<div class="form-group">');
  folderDiv.addChild('<label>Parent Folder</label>');
  const folderSelect = folderDiv.addChild('<select class="form-control" name="parentFolderId">');
  folderSelect.addChild('<option selected value="null"></option>');
  if (folderList) {
    folderList.forEach((folderItem) => {
...
function sndPresetCard(presetArea, value) {
  const card = presetArea.addChild('<div class=\'card\' style=\'min-width: 500px;\'>');
  // Card Title
  card.addChild('<div class="card-header">' + (value.id == null ? '<strong>New Preset</strong>' : '<strong>Preset</strong> ' + value.id) + '</div>\n');
  // card Body + form
  const form = card.addChild('<div class=\'card-body\'>').addChild('<form class="preset-form" data-table="SNDPreset">');
  // Name
  form.addChild('<div class="form-group"><label>Preset Name</label>\n<input class="form-control" name="name" type="text" value="' + value.name + '">\n</div>');
  // Enabled
  form.addChild('<div class="form-group"><label>Preset Enabled</label>\n<input class="form-control" name="enabled" type="checkbox" ' + (value.enabled ? 'checked' : '') + '>\n</div>\n');
  // Existing Data
  const existing = form.addChild('<div class="form-group">');
  existing.addChild('<label>OSC Data</label>');
  if (value.data != '') {
    const data = JSON.parse(value.data);
    for (const [key, value] of Object.entries(data)) {
      const div = existing.addChild('<div class="form-group">');
  ...
  // New Data
  const oscGroup = form.addChild('<div class="form-group">');
...
function oscAddressInputs(oscGroup) {
  ...
  const presetAddress = oscGroup.addChild('<input class="form-control oscInput" name="preset[]" type="text" readonly value="">');

  // first item
  const select = oscGroup.addChild('<select class="form-control presetOption">');
  ...
  const div = oscGroup.addChild('<div class=\'presetDiv\'>'); // next info
  const secondDiv = oscGroup.addChild('<div class=\'presetDiv\'>');
  select.change(function() {
    const options = sndFirstOption[mixer][select.find('option:selected')[0].innerText];// get all info
    ...
    if (options[4]) { // if has second options for that channel/bus etc, show that form
      const secondSelect = div.addChild('<select class="form-control presetOption">'); // create another selection
      ...
      secondSelect.change(function() {
        ...
        const secondOptions = sndSecondOption[secondSelect.find('option:selected')[0].innerText];
        if (options[1]) {
        ...
        const secondNumber = secondDiv.addChild('<input class="form-control presetNumber" type="number" value="' + secondOptions[1] + '" min="' + secondOptions[1] + '" max="' + secondOptions[2] + '" step="' + secondOptions[3] + '">');
      ...
      secondDiv.html('');// clear second div on changes
      const secondNumber = div.addChild('<input class="form-control presetNumber" type="number" value="' + options[1] + '" min="' + options[1] + '" max="' + options[2] + '" step="' + options[3] + '">');
      secondNumber.change(function() {
        presetAddress[0].value = '"' + options[0] + '":{"type":"i", "value":' + secondNumber[0].value + '}';
      });
    } else { // otherwise just show a "true/false" num box
      secondDiv.html('');// clear second div on changes
      const secondNumber = div.addChild('<input class="form-control presetNumber" type="number" min="0" max="1">');
      secondNumber.change(function() {
        if (options[1]) {
...
function sndFaderCard(faderArea, value) {
  const card = faderArea.addChild('<div class=\'card\'>');
  // Card Title
  card.addChild('<div class="card-header">' + (value.id == null ? '<strong>New Fader</strong>' : '<strong>Fader</strong> ' + value.id) + '</div>\n');
  // Card Body + form
  const form = card.addChild('<div class=\'card-body\'>').addChild('<form class="preset-form" data-table="SNDFaders">');
  // Name
...
socket.on('folder', (data) => {
  if ('lxPresetFolders' in data) {
    const faderArea = $('#LXPresetFolderList');
    faderArea.html('');
...
socket.on('preset', (data) => {
  if ('LXPreset' in data) {
    ...
    const presetarea = $('#SNDPresetList');
    presetarea.html('');
...
socket.on('config', (data) => {
  ...
  if (type) {
    const section = $('form.settings-form[data-table=' + type + ']');
    section.html('');
    $.each(data[type], function(key, value) {
      const KeyValue = value.value.replace(/"/g, '&quot;');
