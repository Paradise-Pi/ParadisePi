...
export class InsertConfig1650710286405 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		let count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131FirstUniverse';`
		)
		if (count[0].count < 1) {
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131Universes';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131SourceName';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131Priority';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131Frequency';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'fadeTime';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131Sampler_time';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM lxConfig WHERE key = 'e131Sampler_effectMode';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM sndConfig WHERE key = 'targetIP';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM sndConfig WHERE key = 'mixer';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'deviceLock';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'timeoutTime';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'LXInfo';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
				`INSERT INTO config (key, value, name) VALUES ('LXInfo','', 'LX Additional Info');`
			)
		}
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'SNDInfo';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'LXEnabled';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'LXAdditonalMenu';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
		...
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'SNDEnabled';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
				`INSERT INTO config (key, value, name) VALUES ('SNDEnabled','true', 'Sound Page');`
			)
		}
		count = await queryRunner.query(
			`SELECT COUNT(*) as count FROM config WHERE key = 'AdminEnabled';`
		)
		if (count[0].count < 1) {
			await queryRunner.query(
