...
import { FaCogs, FaSort, FaLevelUpAlt, FaRegFolder, FaRegSave, FaDatabase, FaBars } from 'react-icons/fa'
...
export function AdminNavigation() {
	...
	const database = useAppSelector(state => (state.database ? state.database.message : ''))
		...
		<Navbar height={height} width={{ xs: 200, sm: 200, md: 200 }} p="md" className={classes.navbar}>
			<Navbar.Section grow component={ScrollArea}>
				...
				<NavbarItem
					...
					setActive={setActive}
				/>
				<NavbarItem link="menus" label="Menus" Icon={FaBars} active={active} setActive={setActive} />
				<NavbarItem link="folders" label="Folders" Icon={FaRegFolder} active={active} setActive={setActive} />
				<NavbarItem link="presets" label="Presets" Icon={FaRegSave} active={active} setActive={setActive} />
				<NavbarItem link="faders" label="Faders" Icon={FaLevelUpAlt} active={active} setActive={setActive} />
				<NavbarItem link="sorting" label="Sorting" Icon={FaSort} active={active} setActive={setActive} />
			</Navbar.Section>
