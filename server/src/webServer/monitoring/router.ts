...
export const monitoringRouter = (path: string, method: httpMethods): Promise<monitoringApiResponse> => {
	...
	return new Promise((resolve, reject) => {
		...
		} else if (path.startsWith('/monitoring/variable')) {
			const variableName = path.split('/').pop()
			if (typeof variableName !== 'string')
				return resolve({ statusCode: 400, body: 'BAD REQUEST', contentType: 'text/plain' })
			// Get variable from the database
			return dataSource
				...
				.then(variable => {
					if (!variable) return resolve({ statusCode: 404, body: 'NOT FOUND', contentType: 'text/plain' })
					else {
						return resolve({
							statusCode: 200,
							body: variable.value,
