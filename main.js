...
async function initDatabases() {
  ...
  if (!await knex.schema.hasTable('config')) {
    ...
    await knex('config').insert({key: 'LXAdditonalMenu', value: 'true', name: 'Lighting Special Functions', description: 'Show additional Lighting options such as channel check and keypad', options: '["Show", "Hide"]'});
...
// OSC Setup
let udpPort;
let lastOSCMessage = 0;
let udpStatus = false;
// mixer specifics
const oscPort = {'xair': 10024, 'x32': 10023};
const masterAddress = {'xair': '/lr', 'x32': '/main/st'};
...
function setupE131() {
  ...
  const uCID = Buffer.from(interfaces[Object.keys(interfaces)[0]][0].mac);

  e131Clients = [];
  for (let i = parseInt(LXConfig.e131FirstUniverse); i <= parseInt(LXConfig.e131FirstUniverse)+parseInt(LXConfig.e131Universes)-1; i++) {
  ...
  if (LXConfig.e131Universes > 0) {
    for (let universe = parseInt(LXConfig.e131FirstUniverse); universe <= parseInt(LXConfig.e131FirstUniverse)+parseInt(LXConfig.e131Universes)-1; universe++) {
      sendE131(universe);
    }
  }
}
...
ipcMain.on('sendACN', async (event, args) => {
  if (MAINConfig.deviceLock === 'UNLOCKED') {
    ...
    const channelsValues = args.channelsValues; // Format = {53:244,14:34,56:255}
    const fadeTime = (args.fadeTime !== undefined && args.fadeTime !== null ? args.fadeTime : LXConfig.fadeTime);
