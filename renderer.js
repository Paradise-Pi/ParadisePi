...
window.api.receive('fromOSC', (data) => {
  ...
  if (data.address == '/info') {
    ...
    const master = $('.channel-toggle[data-channel=\'master\']');
    if (data.args[0] === 1) {
    ...
    const meterData = data.parsed;
    $('.fader').each(function(key, fader) {
      if (meterData[this.getAttribute('data-channel')] !== undefined) {
        const value = Math.round(meterData[this.getAttribute('data-channel')]*100);
...
const timeout = { // Black the screen after a timeout
...
$(function() {
  ...
  // Keypad
  const keyOutput = $('#keypad-output');
  const keyFader = $('#intensityFader');
  ...
  $(document).on('input', keyFader, function() {
    const sections = keyOutput.val().split(' ');
